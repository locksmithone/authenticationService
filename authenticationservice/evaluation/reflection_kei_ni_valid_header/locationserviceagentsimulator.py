...
#        self.bnonceTimer = threading.Timer(0, self.scheduleBnonceUpdate)
    ...
    def processBroadcastEcdheSendKerNr(self):
        ...
        # ---------------- Attack Modification ----------------
        # MitM attack: reflection attack. Reflect or replay back the same message received from User agent, but now with valid header
        # and valid payload descriptors.
        # Here, Mallory illustrates playing Alice, the Location Service, and replays the KEi, Ni payload back to user agent.
        # We will compose the message as expected, but will send the KEi, Ni values instead of the KEr, Nr.
        ...
        message = self.sendMessage(constants.HEADER_EXCHANGE_TYPE_SEND_KE_K, constants.HEADER_MESSAGE_TYPE_REQUEST, {constants.PAYLOAD_FIELD_NAME_KER:self.kei, constants.PAYLOAD_FIELD_NAME_NR:self.ni})
