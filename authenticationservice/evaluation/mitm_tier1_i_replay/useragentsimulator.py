...
#        self.resetInstanceVariables()
    ...
    def processBroadcastEcdheReceiveBnonce(self):
        ...
        # Set the received SPIr from the Location Service. SPIi will be set when user agent sends first message.
        # For Bluetooth, this should be done at the BNONCE reception phase.
...
#        if debug: print("User agent: shared secrets computed.")
    ...
    def processTier1PrivacyAuthentication(self):
        ...
        # ---------------- Attack Modification ----------------
        # MitM attack: attempt to replay AUTH_TIER1_i.
        if debug: print("\n\nMitM Attack Mode: Mallory here, impersonating user agent (Bob). Attempting to replay AUTH_TIER1_i...\n\n")
        
        # We will have the user agent, controlled by Mallory, submit some replayed AUTH_TIER1_i.
        # The issue for Mallory is including some ECDHE value that she cannot simply replay, such as the KEr (for which she cannot compute
        # the secret kr). Mallory has to compute her own kr, KEr and give KEr to Bob, and Bob will use that in his AUTH_TIER1_i.
        # Without control of the kr value, Mallory cannot establish an encrypt session with Bob.
        # As such, for simulation purposes, let's substitute the current self.ker for other value, technically obtained
        # from another session.
        self.ker = self.kei # Let's just put the kei value into the ker value, it serves our simulation purposes.
        # ---------------- End Attack Modification ----------------
        ...
        while True:
            ...
            # -------------------------------------------------
                ...
                if status == Status.BNONCE_RECEIVED:
                    ...
                    # TODO: Choose what to do here in case of bad BNONCE. Stop immediately, or proceed to Tier 1 Auth, which will eventually fail.
