...
#        # Responder sends to Initiator:
    ...
    def processTier2PrivacyAuthenticationLPK_LSK(self):
        ...
        # ---------------- Attack Modification ----------------
        # MitM attack: attempt to deceive Bob into thinking he has obtained full authentication with Alice.
        # Mallory, impersonating Alice, cannot compute sAuth. She will just pick some sAuth value.
        ...
        self.sAuth['value'] = Crypto.Hash.SHA256.new(data=b'Ok, this is not random sAuth, but it will work.').digest()
    ...
    def processExchangeAuthenticationJointFactorKeyGeneration(self):
        ...
        if hashlib.sha256(authiReceived).digest() != hashlib.sha256(authiLocal).digest():
            ...
            # ---------------- Attack Modification ----------------
            # MitM attack: attempt to deceive Bob into thinking he has obtained full authentication with Alice.
            # Mallory, impersonating Alice, cannot compute sAuth. She will just pick a random sAuth value.
            # Do not close the connection due to mismatching AUTHi; Mallory expects that, and she attempts to proceed with Bob.
            #return Status.INVALID_JFK_AUTH_PAYLOAD, response
