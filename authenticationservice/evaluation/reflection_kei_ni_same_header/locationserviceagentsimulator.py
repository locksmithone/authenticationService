...
#        self.bnonceTimer = threading.Timer(0, self.scheduleBnonceUpdate)
    ...
    def processBroadcastEcdheSendKerNr(self):
        ...
        # ---------------- Attack Modification ----------------
        # MitM attack: reflection attack. Reflect or replay back the same message received from User agent.
        # Here, Mallory illustrates playing Alice, the Location Service, and replays the KEi, Ni payload back to user agent.
        # We will simply change one field of the header to simulate the replayed message, and transmit the payload received from
        # user agent. The header is the first to be detected as invalid at the other peer.
        ...
        message = self.sendMessage(constants.HEADER_EXCHANGE_TYPE_SEND_KE_K, constants.HEADER_MESSAGE_TYPE_RESPONSE, {constants.PAYLOAD_FIELD_NAME_KER:self.ker, constants.PAYLOAD_FIELD_NAME_NR:self.nr})
