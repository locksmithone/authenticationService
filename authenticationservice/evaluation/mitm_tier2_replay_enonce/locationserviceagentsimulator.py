...
#        # Responder sends to Initiator:
    ...
    def processTier2PrivacyAuthenticationLPK_LSK(self):
        ...
        # ---------------- Attack Modification ----------------
        # MitM attack: attempt to deceive Alice with replayed eNonce messages.
        # Mallory, impersonating Alice, will simply relay eNonces from Bob to Alice.
        if debug: print("\n\nMitM Attack Mode: attempt to deceive Alice by replaying eNonces from Bob...")
        if debug: print("Here, we simulate the attack by having the Location Service compute its LPK/LSK with a different sharedSecret than the one Bob has.\n\n")
        self.sharedSecret = self.ker ** Crypto.Random.random.getrandbits(constants.RANDOM_SECRET_LENGTH_BITS)
