...
#        self.bnonceTimer = threading.Timer(0, self.scheduleBnonceUpdate)
    ...
    def processTier1PrivacyAuthentication(self):
        ...
        # ---------------- Attack Modification ----------------
        # MitM attack: attempt to guess some Nb to compute the AUTH_TIER1_r. First we compute the genuine payload and sign it, and then
        # we compute one with a guessed Nb, without signing again, because Mallory cannot validly sign it. We use the previous (or bogus) signature.
        if debug: print("\n\nMitM Attack Mode: Mallory here impersonating Location Service (Alice). Attempting to guess Nb and computing AUTH_TIER1_r...\n\n")
        # Just generate a random Nb.
        self.currentNb = locationserviceutility.generateNonce(length=self.bnonceLengthBits)
        # Mallory computes an AUTH_TIER1_r payload, but cannot sign it as Alice. Here, we use the genuine signature.
        authTier1r = self.computeAuthTier1r()
