...
#        self.bnonceTimer = threading.Timer(0, self.scheduleBnonceUpdate)
    ...
    def processTier1PrivacyAuthentication(self):
        ...
        # ---------------- Attack Modification ----------------
        # MitM attack: attempt to replay AUTH_TIER1_r generated by Alice.
        if debug: print("\n\nMitM Attack Mode: Mallory here, impersonating Location Service (Alice). Attempting to replay AUTH_TIER1_r...\n\n")
        
        # We will have the Location Service, controlled by Mallory, submit some replayed AUTH_TIER1_r.
        # The issue for Mallory is including some ECDHE value that she cannot simply replay, such as the KEi (for which she cannot compute
        # the secret ki). Mallory has to compute her own ki, KEi and give KEi to Alice, and Alice will use that in her AUTH_TIER1_r.
        # Without control of the ki value, Mallory cannot establish an encrypt session with Alice.
        # As such, for simulation purposes, let's substitute the current self.kei for other value, technically obtained
        # from another session.
        self.kei = self.ker # Let's just put the ker value into the kei value, it serves our simulation purposes.
